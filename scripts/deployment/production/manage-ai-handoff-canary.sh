...
[[ "$action" =~ ^(preflight|open|close|status)$ ]] \
  || fail "action_invalid"
...
if not path.is_file() or path.is_symlink():
    raise SystemExit("AI environment file must be one regular file")
mode = stat.S_IMODE(path.stat().st_mode)
if mode & 0o077:
    raise SystemExit("AI environment file grants group or other access")
...
for raw_line in path.read_text(encoding="utf-8").splitlines():
    ...
    key = key.strip()
    if key in values:
        raise SystemExit(f"duplicate AI environment key: {key}")
...
if any(not values.get(key) for key in required):
    raise SystemExit("required AI Handoff environment key is missing")
enabled = values["AI_HANDOFF_BACKEND_ENABLED"].casefold()
if enabled not in {"true", "false"}:
    raise SystemExit("AI Handoff enabled value must be true or false")
parsed = urlsplit(values["AI_BACKEND_BASE_URL"])
if (
    parsed.scheme != "http"
...
try:
    timeout = float(values["AI_HANDOFF_TIMEOUT_SECONDS"])
...
matches = [
    ...
    if line.lstrip().startswith("AI_HANDOFF_BACKEND_ENABLED=")
]
if len(matches) != 1:
    raise SystemExit("AI Handoff enabled key must occur exactly once")
ending = "\n" if lines[matches[0]].endswith("\n") else ""
lines[matches[0]] = f"AI_HANDOFF_BACKEND_ENABLED={new_value}{ending}"
...
verify_ai_runtime() {
  compose exec -T ai python -c \
    "import os; value=os.getenv('AI_HANDOFF_BACKEND_ENABLED','').strip().lower(); assert value in {'true','false'}; print('ai_handoff_enabled='+value)"
  compose exec -T backend python -c \
...
cleanup_open_failure() {
  ...
  set_enabled false >/dev/null 2>&1 || restored=0
  if [[ "$(env_status 2>/dev/null)" == "false" ]]; then
    recreate_ai >/dev/null 2>&1 || restored=0
  else
    restored=0
  fi
  ...
  if (( restored == 1 )); then
    ...
    compose stop ai >/dev/null 2>&1 || true
    printf 'CANARY_OPEN_FAILURE_FAIL_CLOSED ai=stopped nginx_gate=retained\n' >&2
  fi
  exit "$original_exit"
}
...
on_exit() {
  ...
  if (( exit_code != 0 && close_in_progress == 1 )); then
    ...
    set +e
    compose stop ai >/dev/null 2>&1
    set_enabled false >/dev/null 2>&1 || true
    [[ -f "$nginx_gate_file" ]] || install_gate >/dev/null 2>&1 || true
    printf 'CANARY_CLOSE_FAILURE_FAIL_CLOSED ai=stopped nginx_gate=retained\n' >&2
    exit "$exit_code"
  fi
}
...
close_window() {
  ...
  set_enabled false
  [[ "$(env_status)" == "false" ]] || fail "canary_false_restore_failed"
  recreate_ai
...
case "$action" in
  preflight)
    ...
    [[ "$(env_status)" == "false" ]] \
      || fail "ai_handoff_must_start_disabled"
  ...
  open)
    ...
    set_enabled true
    [[ "$(env_status)" == "true" ]] || fail "canary_true_activation_failed"
    if ! recreate_ai; then
      compose stop ai >/dev/null 2>&1 || true
      set_enabled false || true
      recreate_ai >/dev/null 2>&1 || true
      restore_nginx "$nginx_sha" >/dev/null 2>&1 || true
      fail "ai_recreate_after_activation_failed"
    fi
