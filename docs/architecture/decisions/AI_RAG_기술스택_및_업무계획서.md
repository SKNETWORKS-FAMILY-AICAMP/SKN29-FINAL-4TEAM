...
본 문서는 **'정수기 구독 고객 케어 및 A/S 업무 지원 시스템'** 프로젝트의 AI/RAG 담당자로서 확정한 **최종 기술 스택 명세**와 WBS(Work Breakdown Structure)에 기반한 **주차별/단계별 세부 업무 계획서**입니다.
...
| 제외 기술 | 제외 사유 |
| :--- | :--- |
| 그래프 DB | 데이터 구축 공수 폭발 및 오버 엔지니어링 , RDB 메타 데이터와 하이브리드 검색으로 대체 가능 |
| Ollama | 서버/API 서빙 동시 처리 성능 부족 , 서빙 전용은 vLLM 사용 |
| LlamaIndex | 단순 RAG에는 용이하나, complex agent workflow 및 가드레일 제어에는 LangChain/LangGraph가 우수 |
---



## 1. 2~3주차 주차별 핵심 목표 요약
...
## 2. 일자별 세부 액션 플랜 (Daily Action Plan)
...
## 3. 2~3주차 산출물 및 완료 기준 (Deliverables & Acceptance Criteria)
...
## 4. 백엔드 및 타 역할과의 협업 지점 (Inter-Role Checkpoints)
