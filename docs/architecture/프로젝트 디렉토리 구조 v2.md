...
└─ `/internal/admin/` → Django Admin
   ├─ Public API 계약에서 제외
   ├─ OPERATOR 역할만으로 접근 불가
   └─ 로컬·내부망·VPN·IP allowlist 등 승인된 환경에서만 접근
```
...
│  │  │  ├─ authentication_service.py              # JWT 발급·로그인·Refresh 검증 및 비활성 계정 접근 차단 Service
│  │  │  └─ account_service.py                     # 합성 사용자 조회·수정·비활성화·재활성화 및 Token 폐기 조율 Service
...
│  │  ├─ admin.py                                  # 합성 사용자 생성·조회·수정·비활성화·재활성화와 관리자 권한 제한 설정
...
│  │  │  ├─ audit_event.py                         # 주요 생성·수정·상태 변경과 사용자 계정 변경 이력 Model
│  │  │  └─ ai_execution_log.py                    # AI 단계·모델·프롬프트·처리시간 Model
│  │  ├─ services/                                  # 공통 감사 기록 업무 규칙
│  │  │  ├─ __init__.py                            # Audit Service 패키지 선언
│  │  │  └─ audit_service.py                       # 업무 상태 및 사용자 계정 변경 감사 이벤트 기록 Service
...
| Django App | 핵심 책임 |
|---|---|
| `accounts` | 사용자 역할·가상 로그인·JWT 인증·접근 권한·합성 사용자 계정 수명주기 관리 |
...
## 14.4 2026-07-31 최신 백엔드 산출물 구조

개인 개발문서는 지침·실행 절차·특정 시점 검증·설계·팀 인계의 역할을
분리한다. 같은 내용을 날짜·팀원·Wave별 파일에 반복하지 않고, 현행
문서는 책임과 기능을 드러내는 한글 `snake_case` 파일명을 사용한다.

```text
docs/
├─ api/                                             # Public API 사람용 명세
├─ database/
│  └─ t-005/                                       # 계약·ERD·기계 검증 패키지
├─ handoffs/
│  └─ README.md                                    # 현재 팀 인계 진입점
└─ individual/jiyong/
   ├─ README.md                                    # 최지용 현행 문서 인덱스
   ├─ manuals/
   │  ├─ 20260729_백엔드_api_계약_및_런타임_통합_검증_보고서.md
   │  ├─ 워터브리지_백엔드_설치_마이그레이션_시드_복구_가이드.md
   │  └─ 합성_고객_데모_로그인_가이드.md
   ├─ team_handover/
   │  └─ 백엔드_팀_검토_및_인계_체크리스트.md
   └─ technical/
      ├─ backend/
      │  ├─ 20260730_t005_테이블_구현_및_변경_이력.md
      │  ├─ 20260731_t005_워터브리지_postgresql_통합_검증_보고서.md
      │  ├─ 20260731_백엔드_요청_예외_로그_민감정보_감사_보고서.md
      │  ├─ t005_계정_pk_uuid_jwt_전환_및_롤백_가이드.md
      │  ├─ t005_데이터베이스_스키마_변경_실행_가이드.md
      │  ├─ t005_마이그레이션_불변성_사고_및_복구_보고서.md
      │  ├─ t017a_합성_사용자_계정_관리_설계서.md
      │  ├─ t022_증상_제출_api_설계_및_계약_게이트.md
      │  ├─ 백엔드_api_계약_개발_및_인계_가이드.md
      │  ├─ 백엔드_파이썬_가상환경_재현_가이드.md
      │  └─ 합성_데이터_스키마_적재기_postgresql_검증_가이드.md
      └─ contracts/
         ├─ 백엔드_ai_계약_런타임_통합_미해결_사항.md
         └─ 합성_데이터_픽스처_해시_교차표_검증_보고서.md
...
└─ database/
   ├─ audit_t005_implementation_readiness.py       # T-005 Runtime 준비도 감사
   ├─ check_postgresql_connection.py
   └─ validate_t005_schema.py                     # 계약·Snapshot·완료 Evidence 검증
```

현행 Django Runtime은 `backend/**`, 기계 계약은 `contracts/**`를
기준으로 한다. 루트 `WaterCareBackend/**`와 구형 BAT 파일은 과거
Android 연동 starter 참고본이며 권위 원본이 아니다.

## 14.5 현재 구현 판정

구현 여부는 파일명이나 문서 존재가 아니라 Model·Migration·App 등록,
Route, 같은 Commit의 Runtime 테스트와 PostgreSQL 실행 증거로 판정한다.

| 범위 | 2026-07-31 확인 상태 | 다음 순서 |
| --- | --- | --- |
| T-005 | 계약 테이블 Model·Registry·Migration 32/32, Auditor `READY`, 빈 PostgreSQL·Seed·Importer·전체 회귀 로컬 검증 완료 | 비작성자 독립 재현·외부 소비 검토·PM 공식 승인 |
| T-016 | `waterbridge.public`, Health·Auth Runtime과 공통 실행 환경 동작 | 팀 공용 SHA의 독립 재현 |
| T-017 | 정수 PK·공개 UUID·UUID-only JWT 구현, T-017A 합성 계정 관리 OWNER 설계 완료 | 정책·Migration 검토 후 T-017B/C 순차 구현 |
| T-022 | 문의 생성 Runtime과 증상 제출 API 계약 Gate 존재 | `SUBMIT_SYMPTOM` 첫 수직 Slice 구현·즉시 검증 |
| T-023 | `CANCEL_INQUIRY`, 409·멱등성·Guard 대표 흐름 구현 | PM State 입력에 따른 후속 행동 API 확장 |

최근 기록된 전체 회귀는 SQLite `740 passed, 11 skipped`,
PostgreSQL `751 passed`, Data `67 tests`다. 이 값은 해당 실행
Commit의 증거이며 후보 SHA가 바뀌면 다시 실행한다.

ERD·테이블 명세·API 명세 작성에는 별도 착수 승인을 두지 않는다.
팀원 검토는 소비자 호환성, 독립 재현과 PR 품질 Gate로 수행한다.
