...
└─ individual/jiyong/
   ...
   ├─ manuals/
   │  ├─ 20260729_최지용_Django_PostgreSQL_공유패키지_인계서_v1.3.md  # v1.1·v1.2 이력을 포함한 단일 원본
   │  ├─ 20260729_postgresql_synthetic_handoff_runtime_verification.md
   │  ├─ 20260729_synthetic_customer_auth_alias.md
   │  ├─ 20260727_최지용_Django_PostgreSQL_Migration_검증보고서_v1.0.md
   │  └─ 20260727_최지용_Auth_API_계약_Runtime_정합화_보고서_v1.0.md
   └─ technical/
       ├─ backend/
       │  ├─ 20260729_synthetic_domain_schema_migration.md
       │  ├─ 20260729_synthetic_handoff_importer.md
       ...
       └─ contracts/
          ├─ 20260729_data_qa_fixture_hash_hardening.md
...
| 범위 | 2026-07-29 확인 상태 | 다음 순서 |
| --- | --- | --- |
| T-005 | ERD·테이블 명세는 최지용 확정 산출물, Runtime은 32개 중 10개 | 김은진 Data Owner 검토·PM 병합 후 다음 Model Wave |
| T-016 | PostgreSQL 16.14 연결·전체 Migration 적용·합성 Handoff 격리 DB 검증 | 팀원 로컬 재현 |
| T-017 | User·CustomerProfile·Migration·Seed·Auth 4개 Runtime과 합성 고객 별칭 구현 | 기능 확장 시 계약·테스트 동시 갱신 |
| T-022 | 문의 생성 `START_INQUIRY` 수직 Runtime 구현 | 문진·조회 등 후속 Endpoint를 계약부터 한 단위씩 구현 |
| T-023 | 문의 취소 `CANCEL_INQUIRY`·409·멱등 Replay 구현, 범용 State Engine의 운영 Service 연결은 미완료 | PM이 지정한 다음 Action을 한 수직 단위로 연결 |

현재 후보의 전체 회귀는 Backend `397 passed`, Data `61/61 PASS`,
QA Pipeline 2회 연속 PASS다. Crosswalk v2는 Backend Source 17개와
Fixture Mapping 12개를 검증했지만 김은진 Data Owner 검토 전에는
PM이 `main`에 병합하지 않는다.
