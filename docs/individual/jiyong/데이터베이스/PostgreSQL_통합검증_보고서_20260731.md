# WaterBridge PostgreSQL 통합 검증 보고서
...
> 문서 성격: 이 문서의 `현재`·`현행`은 2026-07-31 실행 스냅샷을 뜻한다.

이 문서는 현재 기본 PostgreSQL 데이터베이스와 팀 재현 절차의 단일
진입점이다. Backend 설치·가상환경 원리는
[백엔드 Python 가상환경 재현 가이드](../개발환경/Django_PostgreSQL_로컬개발환경_설치_실행_복구_가이드.md),
...
이 절은 T-005 착수 전 상태와 32개 테이블 구현 후 검증 상태를 비교한다.
Wave별 세부 Model·Migration·제약 결정은
[T-005 테이블 구현 및 변경 이력](Django_PostgreSQL_테이블_구현_변경이력_20260730.md)을
단일 원본으로 사용한다.
...
[T-005 Migration 불변성 사고 및 복구 보고서](PostgreSQL_마이그레이션_불변성_사고_복구_보고서.md)에
보존한다.
...
| Gate | 현재 | 완료 조건 | 책임 |
| --- | --- | --- | --- |
| 구현·PostgreSQL·Seed·Importer | 통과 | 현행 증거 유지 | Backend·Database(최지용) |
| `non_author_review_confirmed` | 대기 | 비작성자가 같은 후보 SHA에서 빈 PostgreSQL·Seed·Importer·회귀 재현 | Data·QA(김은진 또는 지정 리뷰어) |
| 외부 소비 검증 | 대기 | Web·Mobile·AI가 UUID·API·Vector·Evidence 계약 소비 확인 | 각 소비 담당 |
| PM 계약 완료 리뷰 | 대기 | 물리 계약·독립 증거·변경 경계 승인 및 `main` 기준선 고정 | PM·계약(윤승혁) |
...
| 날짜 | 변경 |
| --- | --- |
| 2026-07-31 | WaterBridge DB 전환과 T-005 32개 테이블 구현 이후 PostgreSQL·Seed·Importer 최종 검증 기준을 기록 |
