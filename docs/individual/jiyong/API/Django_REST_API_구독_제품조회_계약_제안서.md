# Django REST API 구독·제품 조회 계약 제안서
...
>
> 현재 상태: 계약·테스트 제안 단계, Runtime 미구현
>
> 내부 상태 코드: `SAFE_CONTRACT_TEST_PROPOSAL_ONLY`
>
> 구현 여부: **미구현 — 계약·테스트 제안만 작성했으며 Runtime·Migration은 변경하지 않았다.**

## 1. 목적과 안전 범위

구독·제품 조회 업무(T-018)의 첫 구현 단위를
`GET /api/v1/me/subscriptions` 단일 읽기 Slice로 제한한다.
...
| 사람용 기준 | [API 명세](../../../api/waterbridge_api_specification.md) | `API-SUB-001` 설계 기준 존재 |
...
CareRecord를 최신으로 볼지 확정된 집계 규칙이 없다. 다음 규칙을 Data·QA
담당자와 검토한 뒤 nullable Projection을 추가한다.
...
| 역할(담당자) | 확인할 내용 | 반환 |
| --- | --- | --- |
| Data·QA(김은진) | 본인 범위·MVP 제품 필터·최근 관리일 집계·QA Matrix | 승인·변경 요청과 재현 Case |
| PM·계약(윤승혁) | 공개 필드·역할·오류·T-018 부분 완료 경계 | 승인·보류·변경 요청 |
| Backend·Database(최지용) | 승인 결과를 계약 Diff로 반영하고 작은 Slice 구현 | 명령·Exit code·동일 PR 변경 묶음 회귀 |
