...
- 현재 상태: `LOCAL_REVIEW_PACKET_READY_NOT_PUBLISHED`
...
팀 Git 기준은 `각자 Branch Push → PR 검토 → 윤승혁 PM의 main 병합 →
전 팀원 main 최신 Pull`이다. 2026-08-01에 작성한 검토 패킷·T-018
제안·README·이 체크리스트는 아직 로컬 미커밋·미Push다. 팀원이
검토하기 전 문서만 `jiyong`에 Push하고 PR 링크·검토 대상 파일 목록을
전달한다. Commit ID는 일상 인계의 필수값으로 사용하지 않는다.
...
3. 테스트 개수만 전달하지 않고 작업 Branch, PR, 실행 환경, 명령,
   Exit code, 실행 시각을 함께 남긴다.
4. PM의 PR 병합 완료 통지 후 각 담당자가 최신 `main`을 Pull한 상태를
   팀 공용 기준선으로 사용한다.
5. 기술 구현 완료와 Owner Review·PM 공식 완료 판정을 구분한다.
6. 실제 개인정보, 운영 Dump, `.env`, Token, Secret, 개인 PC 절대경로를
   Commit이나 인계 문서에 넣지 않는다.

## 2. 최지용의 현재 협업·검토 요청 우선순위

### 2.1 기준선과 완료 경계

| 항목 | 현재 값 | 해석 |
|---|---|---|
| 팀 공용 기준 | PM 병합 완료 후 최신 `main` Pull | 개인 Branch나 미병합 PR를 공용 기준으로 사용하지 않음 |
| 최지용 전달 단위 | `jiyong` Push와 PR | PR 링크·변경 파일·검증 결과를 함께 전달 |
| 후보 DB | `waterbridge.public`, 물리 32·Active 13·Target-only 19 | PM 병합 전 팀 공용 기준으로 확대하지 않음 |
| 실행 검증 기준 | 검증 명령·환경·Exit code가 함께 기록된 변경 묶음 | 과거 테스트 수치를 현재 코드의 자동 완료 증거로 사용하지 않음 |
| T-005 | `TECHNICALLY_COMPLETE_REVIEW_PENDING` | 비작성자 독립 재현·PM 공식 판정 필요 |
| T-017 | `OWNER_IMPLEMENTATION_READY` | Auth 기반은 준비됐지만 T-017A 리뷰와 B/C 구현은 별도 |
| T-017A | `OWNER_DESIGN_READY_REVIEW_PENDING` | 윤승혁 정책·김은진 Migration/QA 결정 필요 |
| T-022 Submit | `OWNER_DESIGN_READY_CONTRACT_GATES_PENDING` | OpenAPI·ADHOC Projection·AI 경계 결정 전 Runtime 금지 |
| T-018 | `SAFE_CONTRACT_TEST_PROPOSAL_ONLY` | 최소 GET 제안만 완료, 계약·Runtime·최근 관리일 규칙 미확정 |

### 2.2 지금 요청해야 할 항목

| 우선 | 검토·협업 요청 | 요청 대상 | 최지용이 제공할 입력 | 반환받을 증거 | 현재 상태 |
|---:|---|---|---|---|---|
| 1 | T-005 비작성자 독립 재현 | 김은진 또는 PM 지정 비작성자 | `jiyong` Push·PR 링크, 재현 가이드, 기존 작성자 증거 | 새 DB 32/32, Seed 2회차 신규 0, Importer Replay, PostgreSQL 전체 회귀 | `REQUEST_AFTER_DOC_PUSH` |
| 2 | T-005 공식 완료·`main` 병합 판정 | 윤승혁(PM) | 1의 독립 증거와 계약 Diff 0 | 승인·보류 이유, 병합 PR, 전 팀원 `main` Pull 요청 | `WAITING_FOR_1` |
| 3 | T-017A OWNER 설계 리뷰 | 윤승혁·김은진 | [T-017A 설계서](../technical/backend/t017a_합성_사용자_계정_관리_설계서.md)와 [검토 입력 패킷](../../../handoffs/20260801_t017a_t022_검토_입력_패킷.md) | 정책 결정, Migration·backfill·감사·QA 변경 요청 | `PACKET_READY_NOT_PUBLISHED` |
| 4 | T-022 Slice A 계약 Gate | 윤승혁·김은진·이동윤 | [T-022 설계서](../technical/backend/t022_증상_제출_api_설계_및_계약_게이트.md)와 검토 입력 패킷 | Path·Body·State·ADHOC Projection·AI Slice B 결정 | `PACKET_READY_NOT_PUBLISHED` |
| 5 | T-018 최소 GET 계약·테스트 검토 | 김은진·윤승혁 | [T-018 제안서](../technical/backend/t018_구독_제품_조회_api_계약_제안서.md) | 본인 범위·MVP 필터·최근 관리일 집계·공개 필드·부분 완료 결정 | `PROPOSAL_READY_NOT_PUBLISHED` |
| 6 | Web·Mobile 소비 검증 | 한예나·양정현 | PM 병합 완료와 최신 `main` Pull, 확정 OpenAPI | UUID·Auth·401/403·409/422·DTO·실제 API Smoke | `WAITING_FOR_MAIN_AND_RUNTIME` |
| 7 | AI Runtime·Evidence 소비 검증 | 이동윤 | 승인된 T-022 Slice B와 Backend Adapter가 포함된 PR·병합 상태 | Schema Parity·Timeout·재처리·Evidence E2E | `WAITING_FOR_SLICE_B_DECISION` |

T-017A, T-022, T-018 검토 요청은 문서 게시 뒤 동시에 보낼 수 있다.
다만 최지용의 구현은 승인된 작업 하나만 선택해
`계약 → 계약 검증 → Runtime → 집중 검증 → 전체 회귀` 순서로 진행한다.

### 2.3 최지용 발송·회수 체크리스트

- [x] T-017A·T-022 담당자별 결정 항목과 반환 형식 작성
- [x] T-018 최소 GET 요청·응답·권한·오류·테스트 Matrix 작성
- [x] 이 체크리스트에 현재 담당자·선행 Gate·완료 경계 반영
- [ ] 관련 문서만 별도 Commit으로 `jiyong`에 Push
- [ ] PR의 변경 파일·CI 상태와 `main...jiyong` 차이 재확인
- [ ] 검토자에게 PR 링크·대상 문서·응답 기한·반환 형식 전달
- [ ] `APPROVE / HOLD / CHANGE_REQUEST` 회신을 담당자별로 기록
- [ ] 승인된 항목 하나만 다음 구현 Wave로 선택
- [ ] 구현 뒤 같은 PR 변경 묶음에서 PostgreSQL·권한·계약·전체 회귀 검증

문서가 아직 원격에 없으므로 `jiyong` Push와 PR 변경 파일 확인 전에는
새 패킷이 팀 공유본에 포함됐다고 설명하면 안 된다.

### 2.4 담당자별 현재 요청 요약

| 대상 | 주관 영역 | 현재 요청 | 반환해야 할 핵심 증거 |
|---|---|---|---|
| 김은진 | `data/**`, Data QA·비작성자 검증 | T-005 독립 재현, T-017A Migration/QA, T-022 DB 불변식, T-018 조회·최근 관리일 검토 | 독립 환경·명령·Exit code·QA 결정·변경 요청 |
| 윤승혁(PM) | 상태 계약·통합·병합 Gate | T-005 공식 판정, T-017A 정책, T-022 State·Path, T-018 공개 계약 결정 | 승인/보류/변경 요청, 결정 근거, 병합 PR·`main` Pull 안내 |
| 한예나 | `web/**` | PM `main`과 실제 Runtime이 나온 뒤 Web 소비 검증 | Test·Lint·Build, 실제 Auth/API Smoke, DTO·오류·식별자 차이 |
| 양정현 | `mobile/**` | PM `main`과 실제 Runtime이 나온 뒤 Mobile 소비 검증 | Unit·Lint·두 App Build, Emulator Smoke, DTO·계약 차이 |
| 이동윤 | `ai/**`, `contracts/ai/**` | 지금은 T-022 Slice B 경계 검토, 이후 AI Runtime·Evidence 소비 검증 | 계약 Parity, dispatch·Timeout·재처리 결정, 최신 `main` 통합 E2E 결과 |

### 2.5 WBS 기준 최지용 공식 작업일정
...
|---|---|---|---|---|---|---|---|
| `T-005` 데이터 설계 | 2026-07-22 | 2026-07-23 | 최지용<br>백엔드·데이터베이스 | 윤승혁·김은진 | T-002, T-004 | 진행 중 | 공식 기간 경과·비작성자 재현과 PM 판정 필요 |
| `T-016` Backend 공통 | 2026-07-23 | 2026-07-23 | 최지용<br>백엔드·데이터베이스 | 김은진 | T-004 | 진행 중 | 공식 기간 경과·팀 환경 재현 증거 필요 |
| `T-017` 사용자·권한 | 2026-07-24 | 2026-07-27 | 최지용<br>백엔드·데이터베이스 | 김은진 | T-005, T-016 | 미착수 | 공식 기간 경과·현재 Auth 증거와 공식 상태 분리 |
| `T-022` 문의·증상 제출 | 2026-07-28 | 2026-07-29 | 최지용<br>백엔드·데이터베이스 | 김은진 | T-005, T-016, T-017 | 미착수 | 공식 기간 경과·계약 Gate 재계획 필요 |
| `T-017A` 계정 관리 설계 | 2026-07-30 | 2026-07-30 | 최지용<br>백엔드·데이터베이스 | 윤승혁·김은진 | T-005, T-017 | 미착수 | 공식 기간 경과·정책·Migration 검토 대기 |
| `T-023` State Machine API | 2026-07-30 | 2026-07-31 | 최지용<br>백엔드·데이터베이스 | 김은진 | T-002, T-005, T-016, T-022 | 미착수 | T-022 승인·구현 전 추가 Action 확대 금지 |
...
겹친다. 최지용이 두 Runtime을 동시에 진행하지 않는다. 윤승혁 PM이
우선순위와 새 기간을 결정하기 전에는 T-022·T-017A·T-018 검토를
병렬로 요청하되, 구현은 승인된 한 작업만 진행한다.
...
### 2.6 협업·검토 요청일과 회신 시점

WBS에는 설계 검토자의 별도 작업일이 없다. 아래 `요청·회신 시점`은
WBS 선행관계를 지키기 위한 착수 조건이다. 검토 작업 자체의 시작일과
종료일은 WBS에 없으므로 PM이 정하기 전까지 `TBD`이며, 과거 WBS 날짜를
새 검토 마감일로 해석하지 않는다.

| 요청 항목 | 연결 WBS | WBS 시작일 | WBS 종료일 | 일정 성격 | 실제 착수 조건 | 검토 일정 | 요청 대상 |
|---|---|---|---|---|---|---|---|
| T-005 독립 재현 | T-005 | 2026-07-22 | 2026-07-23 | 공식 기간 경과 | `jiyong` 문서 Push·PR 확인 | `TBD` — PM 지정 필요 | 김은진 또는 PM 지정 비작성자 |
| T-005 공식 판정 | T-005 | 2026-07-22 | 2026-07-23 | 별도 판정 기간 WBS 미명시 | 독립 재현 Evidence 반환 | `TBD` — PM 결정 | 윤승혁 |
| T-017A 정책·Migration 리뷰 | T-017A | 2026-07-30 | 2026-07-30 | 공식 기간 경과 | 설계서·검토 패킷 PR 게시 | `TBD` — T-017B 전 확정 필요 | 윤승혁·김은진 |
| T-022 Slice A 계약 리뷰 | T-022 | 2026-07-28 | 2026-07-29 | 공식 기간 경과 | 설계서·검토 패킷 PR 게시 | `TBD` — T-023 전 확정 필요 | 윤승혁·김은진 |
| T-022 Slice B AI 경계 | T-025 연계 | 2026-07-30 | 2026-07-31 | 공식 기간 경과·T-023과 중첩 | 예비 검토는 패킷 게시 후, 최종 승인은 Slice A 확정 후 | `TBD` — PM 재계획 필요 | 이동윤 |
| T-018 최소 GET 리뷰 | T-018 | 2026-07-31 | 2026-08-03 | 공식 기간 내 | T-018 제안서 PR 게시 | `TBD` — Runtime 전 확정 | 김은진·윤승혁 |
| Web 문의·상태 소비 | T-038 | 2026-07-27 | 2026-07-28 | 선행 T-023보다 빠른 충돌 일정 | T-023 Runtime 병합·최신 `main` Pull | `TBD` — PM 재계획 필요 | 한예나·윤승혁·최지용 |
| Mobile 고객 소비 | T-033 | 2026-08-11 | 2026-08-12 | 공식 예정 | T-018·T-020·T-045 완료 | WBS 공식 기간 사용 | 양정현·윤승혁·최지용 |
| Mobile 기사·AI 근거 소비 | T-042 | 2026-08-07 | 2026-08-10 | 선행 T-030B보다 빠른 충돌 일정 | T-030B·T-041·T-045 완료 | `TBD` — PM 재계획 필요 | 양정현·이동윤·최지용 |
| AI Runtime·Evidence 소비 | T-025·T-028B | 2026-07-30 | 2026-08-14 | 일부 경과·T-028A와 중첩 | T-023·T-028A 완료와 계약 확정 | `TBD` — 선행 완료 뒤 확정 | 이동윤·김은진·최지용 |
...
|---:|---|---|---|---|---|
| G-1 | 2026-08-01 | 2026-08-01 | 최지용 | 검토 패킷·T-018 제안·체크리스트 검증 후 `jiyong` Push·PR 갱신 | PR 링크·변경 파일·링크 검증 결과 공유 |
| G-2 | `TBD` | `TBD` | 김은진 또는 지정 비작성자 | 격리 DB/Volume에서 T-005 Migration·Seed 2회·Importer Replay·PostgreSQL 회귀 | 명령·Exit code·32/32·Seed 신규 0·Replay 결과 |
| G-3 | `TBD` | `TBD` | 윤승혁 | G-2 증거와 계약을 검토하고 승인·보류·PR 병합 결정 | 병합 PR·결정 이유·전 팀원 `main` Pull 안내 |
| G-4A | `TBD` | `TBD` | 윤승혁·김은진 | T-017A 정책·Migration·감사 QA 검토 | T-017B/C 착수 결정 |
| G-4B | `TBD` | `TBD` | 윤승혁·김은진 | T-022 Slice A State·OpenAPI·DB 불변식 검토 | T-023 착수 결정 |
| G-4C | `TBD` | `TBD` | 김은진·윤승혁 | T-018 본인 범위·공개 필드·최근 관리일 검토 | T-018 Runtime 착수 결정 |
| G-4D | `TBD` | `TBD` | 이동윤 | T-022 Slice B AI Schema·dispatch·Timeout·재처리 경계 예비 검토 | G-4B 승인 뒤 최종 AI 계약 결정 |
| G-5 | `TBD` | `TBD` | 윤승혁·최지용 | G-4A/B/C 중 선행 Gate가 닫힌 작업 하나만 선택 | PM 재계획 시작·종료일과 선택 작업 기록 |

G-4A/B/C는 검토만 병렬로 요청할 수 있다. 구현은 G-5에서 선택한 한
트랙만 수행하며, 다른 트랙은 첫 작업의 검증과 PR 병합이 끝난 뒤 다시
G-5로 돌아와 선택한다. G-4D 예비 검토는 다른 검토와 병렬로 가능하지만
최종 승인과 Runtime 착수는 G-4B Slice A 확정 뒤에 한다. G-4D는
T-017A·T-018을 차단하지 않는다.

#### 2.7.2 Backend·소비 영역 WBS 실행 순서

| 순서 | WBS | 시작일 | 종료일 | 주담당·협업 | 선행·착수 조건 | 협업 작업과 반환 | 일정 판정 |
|---:|---|---|---|---|---|---|---|
| 1 | T-005 | 2026-07-22 | 2026-07-23 | 최지용 / 윤승혁·김은진 | T-002·T-004 | 설계 계약·독립 재현·PM 판정 | 경과·G-2/G-3 재계획 |
| 2 | T-016 | 2026-07-23 | 2026-07-23 | 최지용 / 김은진 | T-004 | Backend 환경·공통 응답·오류 재현 | 경과·완료 증거 확인 필요 |
| 3 | T-017 | 2026-07-24 | 2026-07-27 | 최지용 / 김은진 | T-005·T-016 | 역할별 Auth·조회 범위 검증 | 경과·공식 상태 재판정 |
| 4A | T-017A | 2026-07-30 | 2026-07-30 | 최지용 / 윤승혁·김은진 | T-005·T-017, G-4A | 정책·Migration·감사 설계 확정 | 경과·`TBD(PM)` |
...
| 5A | T-022 | 2026-07-28 | 2026-07-29 | 최지용 / 김은진 | T-005·T-016·T-017, G-4B | Active 13·원문 불변·409·Replay 검증 | 경과·`TBD(PM)` |
...
| 6A | T-018 | 2026-07-31 | 2026-08-03 | 최지용 / 김은진 | T-005·T-016·T-017, G-4C | 제품·구독·최근 관리일·비노출 검증 | T-017B와 8월 3일 중첩·PM 선택 필요 |
...
| 7A | T-025 | 2026-07-30 | 2026-07-31 | 이동윤 / 김은진·최지용 | T-006·T-015·T-023, G-4D | Orchestrator·Slice B Schema·Timeout 검증 | T-023과 동일 기간 충돌·`TBD(PM)` |
...
| 9A | T-038 | 2026-07-27 | 2026-07-28 | 한예나 / 최지용·윤승혁 | T-023·T-045 | 문의 목록·상태 소비 검증 | T-023보다 빠름·`TBD(PM)` |
| 9B | T-030A | 2026-08-18 | 2026-08-18 | 이동윤 / 김은진·윤승혁 | T-023·T-029 | 상담 AI 요약 초안 계약 반환 | T-039의 선행인데 후행 날짜 |
| 9C | T-039 | 2026-07-29 | 2026-07-30 | 한예나 / 최지용·윤승혁 | T-030A·T-038 | 상담 상세·AI 초안 소비 검증 | T-030A보다 빠름·`TBD(PM)` |
...
| 10B | T-042 | 2026-08-07 | 2026-08-10 | 양정현 / 최지용·이동윤 | T-030B·T-041·T-045 | 기사 리포트·근거 소비 검증 | T-030B보다 빠름·`TBD(PM)` |
...
|---:|---|---|---|---|---|---|---|
| 11 | T-046 | 2026-08-21 | 2026-08-24 | 최지용 / 전 영역 담당 | T-025·T-033·T-038·T-042·T-044 | 최신 `main`에서 고객→상담사→기사 E2E | 앞선 충돌의 도미노 영향 확인 |
...
| 15 | T-048 | 2026-08-25 | 2026-08-26 | 김은진 / 전 팀원 | T-047·T-047A·T-049~T-051 | 모든 영역 결과를 취합해 종합 보고 | 선행보다 빠름·`TBD(PM)` |
...
## 3. 김은진 — Data QA·비작성자·계약 검토

> **3~7장 일정 표기 원칙**
> `WBS 시작일·종료일`은 WBS 원문의 연계 작업기간이며 협업 검토의 새
> 마감일이 아니다. `TBD`는 일정 누락이 아니라 윤승혁 PM의 재계획이
...
#### 3.2.1 T-005 비작성자 독립 재현

- **연결 WBS:** `T-005`
- **WBS 공식 작업일:** `2026-07-22 ~ 2026-07-23`
- **실제 협업 작업일:** `TBD — 윤승혁 PM 지정 필요`
- **일정 상태:** 공식기간 경과·독립 재현 기간 WBS 미명시
- **착수 조건:** `jiyong` 문서 Push·PR 확인, 새 worktree·빈 PostgreSQL 준비
- **완료·반환:** 명령·Exit code·32/32·Seed 2회·Importer Replay·전체 회귀 결과

김은진이 검토 후보 변경의 비작성자인 경우에만 직접 수행한다. 김은진이
작성에 참여했다면 윤승혁 PM이 지정한 다른 비작성자가 수행한다.

- [ ] 최지용의 기존 DB·Container·Volume을 재사용하지 않는 새 환경 준비
- [ ] `jiyong`으로 전환한 뒤 원격 최신 변경 Pull과 작업트리 clean 확인
- [ ] 빈 PostgreSQL에서 Migration 후 T-005 물리 계약 32/32 확인
- [ ] 기본 Seed 2회 실행 후 2회차 비의도 신규 생성 0 확인
- [ ] 격리 DB에서 Importer 최초 실행·Replay 결과 동일 확인
- [ ] 같은 최신 `jiyong` Pull 상태에서 PostgreSQL 전체 회귀와 Data QA 실행
- [ ] 작성자 증거와 독립 증거를 구분해 반환

실행 명령과 DB 안전 수칙은
[WaterBridge 설치·Migration·Seed·복구 가이드](../manuals/워터브리지_백엔드_설치_마이그레이션_시드_복구_가이드.md)를
사용한다. 기본 `waterbridge`에서 합성 Importer를 실행하지 않는다.

#### 3.2.2 T-017A Migration·QA 검토

- **연결 WBS:** `T-017A`
- **WBS 공식 작업일:** `2026-07-30 ~ 2026-07-30`
- **실제 협업 작업일:** `TBD — 윤승혁 PM 재계획 필요`
- **일정 상태:** 공식기간 경과·별도 QA 검토일 WBS 미명시
- **착수 조건:** T-005·T-017 완료 증거와 설계서·검토 패킷 PR 게시
...
#### 3.2.3 T-022 Slice A DB·QA 검토

- **연결 WBS:** `T-022`
- **WBS 공식 작업일:** `2026-07-28 ~ 2026-07-29`
- **실제 협업 작업일:** `TBD — 윤승혁 PM 재계획 필요`
- **일정 상태:** 공식기간 경과·별도 DB·QA 검토일 WBS 미명시
- **착수 조건:** T-005·T-016·T-017 완료와 Slice A 계약 패킷 게시
- **완료·반환:** Active 13·원문 불변·동시성·Replay·rollback QA Matrix

- [ ] Active 13을 유지하고 Target-only `QuestionnaireSession`을 생성하지 않는 Projection 검토
- [ ] 최초 `Inquiry.raw_text` 불변과 추가 입력 미수용 경계 검토
- [ ] 상태·버전·이력·멱등 기록의 단일 Transaction·rollback 검토
- [ ] 동시 요청·409·Replay·실패 후 반쪽 데이터 0 Case 검토

#### 3.2.4 T-018 최소 GET 검토

- **연결 WBS:** `T-018`
- **WBS 공식 작업일:** `2026-07-31 ~ 2026-08-03`
- **실제 협업 작업일:** `TBD — Runtime 착수 전 완료 필요`
- **일정 상태:** 공식 작업 창이나 T-017B와 2026-08-03 중첩·PM 우선순위 필요
- **착수 조건:** T-005·T-016·T-017 완료와 최소 GET 제안서 PR 게시
...
#### 3.2.5 합성 데이터 Owner Review

- **연결 WBS:** `T-013`, `T-014`
- **WBS 공식 작업일:** `T-013: 2026-07-22 ~ 2026-07-24`, `T-014: 2026-07-20 ~ 2026-07-22`
- **실제 협업 작업일:** `TBD — 윤승혁 PM 재계획 필요`
- **일정 상태:** T-014가 선행 T-013 종료보다 빠른 WBS 충돌·Owner Review 기간 미명시
...
```text
[김은진 Backend 협업 검토]
scope=T005_REPRO | T017A_MIGRATION_QA | T022_DB_QA | T018_READ_QA
decision=APPROVE | HOLD | CHANGE_REQUEST
base_branch=main
main_pull_status=UP_TO_DATE | NEEDS_PULL
source_branch=jiyong
...
합성 데이터 Owner Review는 추가로 아래 세부 필드를 반환한다.
...
- [T-017A 합성 사용자 계정 관리 설계서](../technical/backend/t017a_합성_사용자_계정_관리_설계서.md)
- [T-022 증상 제출 API 설계·계약 Gate](../technical/backend/t022_증상_제출_api_설계_및_계약_게이트.md)
- [T-018 구독·제품 조회 API 계약 제안서](../technical/backend/t018_구독_제품_조회_api_계약_제안서.md)
...
#### 4.1.1 T-005 공식 완료·병합 판정

- **연결 WBS:** `T-005`
- **WBS 공식 작업일:** `2026-07-22 ~ 2026-07-23`
- **실제 협업 작업일:** `TBD — 비작성자 독립 재현 반환 후`
- **일정 상태:** 공식기간 경과·PM 판정 기간 WBS 미명시
- **착수 조건:** 비작성자 재현·Data Owner Review·Backend·PostgreSQL 증거 회수
- **완료·반환:** 승인·보류 사유, 병합 PR, 전 팀원 최신 `main` Pull 안내

- [ ] T-005 비작성자 증거와 작성자 증거를 구분해 공식 완료 여부 판정
- [ ] T-005 `32/32` 기술 구현과 공식 WBS 완료 Gate를 분리해 판정

#### 4.1.2 T-017A·T-017B·T-017C 계정 정책·구현 순서 결정
...
- **착수 조건:** T-005·T-017 완료와 김은진 Migration·QA 회신
...
#### 4.1.3 T-022·T-023 문의·Workflow 계약 결정

- **연결 WBS:** `T-022`, `T-023`
- **WBS 공식 작업일:** `T-022: 2026-07-28 ~ 2026-07-29`, `T-023: 2026-07-30 ~ 2026-07-31`
- **실제 협업 작업일:** `TBD — 윤승혁 PM 재계획 필요`
- **일정 상태:** 공식기간 경과·T-022 PM 검토일은 WBS 미명시
- **착수 조건:** T-005·T-016·T-017 완료와 Slice A DB·QA 회신
- **완료·반환:** Path·Body·State·409·ADHOC·다음 Action 정책 결정

윤승혁은 이 섹터의 WBS 공식 협업자가 아니라 운영상 계약 결정자다.

- [ ] T-022 `submitSymptom` Path·Body·응답·State 전이·409 정책 결정
- [ ] ADHOC 제출을 상태·이력 Projection으로 볼지 물리 저장할지 결정
- [ ] 다음 Workflow Action의 Source·Target·Role·Guard·Reopen 정책 확정

#### 4.1.4 T-018 공개 조회 계약 결정

- **연결 WBS:** `T-018`
- **WBS 공식 작업일:** `2026-07-31 ~ 2026-08-03`
- **실제 협업 작업일:** `TBD — Runtime 착수 전 결정`
- **일정 상태:** T-017B와 2026-08-03 중첩·PM 우선순위 필요
- **착수 조건:** 최소 GET 제안과 김은진 QA 회신
- **완료·반환:** CUSTOMER 범위·공개 필드·최근 관리일·부분 완료 경계

윤승혁은 T-018의 WBS 공식 협업자가 아니라 운영상 공개 계약 결정자다.

- [ ] T-018 공개 필드·CUSTOMER 전용·부분 완료 경계 결정
- [ ] T-018 최근 관리일 집계가 확정되기 전 완료 처리하지 않음

#### 4.1.5 Branch·PR 병합 Gate

- **연결 WBS:** 독립 WBS Task 없음
- **WBS 공식 작업일:** `WBS 미명시`
- **실제 협업 작업일:** 각 담당자 PR 검토 완료 후
- **일정 상태:** 운영 Gate·Commit ID 제출 불필요
- **착수 조건:** Owner Review·CI·계약·테스트·비밀값 비포함 확인
- **완료·반환:** 병합 PR·병합 여부와 최신 `main` Pull 명령

- [ ] 각 주담당자의 Branch·PR·Owner Review·CI·검증 결과 확인
- [ ] Source Hash·Data QA·Backend·PostgreSQL 검증이 같은 기준선인지 확인
- [ ] 계약 밖 지원 Runtime 테이블의 분류와 소유 책임 확정
- [ ] 민감정보·환경파일·DB Dump가 변경 목록에 없는지 확인
- [ ] 병합 후 팀에 병합 PR과 최신 `main` Pull 명령 전달
...
```text
[윤승혁 PM 회신]
review_scope=T005_OFFICIAL | T017A_POLICY | T022_CONTRACT | T018_PUBLIC_CONTRACT
decision=APPROVE | HOLD | CHANGE_REQUEST
base_branch=main
reviewed_branches=<Branch 목록>
reviewed_prs=<PR 목록>
data_owner_review=APPROVED
technical_gate_results=<실행 명령·Exit code·결과>
merged_prs=<PR URL 또는 병합 기록>
merge_status=MERGED | NOT_MERGED | CHANGES_REQUESTED
main_pull_required=true | false
...
#### 5.1.2 T-038 상담 대기 문의·상태 소비

- **연결 WBS:** `T-038`
- **WBS 공식 작업일:** `2026-07-27 ~ 2026-07-28`
- **실제 협업 작업일:** `TBD — 윤승혁 PM 재계획 필요`
- **일정 상태:** 선행 T-023 공식일보다 빠른 WBS 충돌
- **착수 조건:** T-023·T-045 완료와 Runtime 병합 후 최신 `main` Pull
...
#### 5.1.3 T-039·T-040·T-041 상담 상세·결과·방문 일정
...
각각 병합된 뒤 최신 `main`에서 수행한다. T-022·T-018 제안서를 Mobile
확정 DTO로 선반영하지 않는다.

### 6.1 요청 작업

#### 6.1.1 T-045 Mobile 공통 계약·역할 라우팅
...
#### 6.1.2 T-033~T-037 고객 Mobile 화면
...
- **착수 조건:** T-045와 각 화면별 선행 Task 완료 후 최신 `main` Pull
...
#### 6.1.3 T-042·T-043 방문기사 Mobile 화면
...
#### 7.1.1 지금 요청 — T-022 Slice B 예비 계약 검토
...
- **착수 조건:** T-017A·T-022 검토 패킷 게시, 최종 결정은 Slice A 승인 후
...
#### 7.1.2 T-025·T-032 Orchestrator·Timeout Runtime
...
#### 7.1.3 T-026~T-031 Evidence·역할별 결과·안전성
...
- [ ] 개인 절대경로·과거 Port·과거 Branch·PR 상태가 남은 문서 정리
- [ ] 최신 `main`에서 AI·Backend·Web·Mobile 통합 E2E와 결함 목록 반환
...
```text
최지용: 2026-08-01 문서 묶음 링크·경계 검증
→ 문서 전용 Commit·jiyong Push·PR 변경 파일 확인
→ 지정 비작성자: 새 worktree·새 PostgreSQL 환경에서 T-005 독립 재현
→ 윤승혁 PM: T-005 공식 완료·main 병합 여부 결정
→ 병합 대기 중 T-017A·T-022·T-018 문서 검토 회신 수집
→ 최지용: 승인된 작업 한 개만 계약→검증→Runtime→검증
→ 같은 PR 변경 묶음의 PostgreSQL·권한·전체 회귀와 비작성자 증거 반환
→ PM 병합 완료 통지 후 전 팀원 main 최신 Pull
→ Web·Mobile·AI 소비자 Smoke와 결함 반환
```

세부 순서:

1. T-005 독립 재현은 작성자 환경과 분리한다.
2. T-017A·T-022·T-018 문서 검토는 병렬 요청할 수 있다.
3. 구현은 T-017A·T-022·T-018 중 선행 Gate가 닫힌 하나만 먼저 선택한다.
4. T-018 Runtime은 최소 GET 계약 승인을 받아도 T-022 Slice A와 같은
   작업 단위에 섞지 않는다.
5. Web·Mobile 전체 소비 검증과 AI 통합 E2E는 PM 병합 후 `main`에서만
   수행한다.
...
| 2026-08-01 T-017A·T-022 검토 입력 패킷 | 담당자별 결정·회신 형식 | 2장·3장·4장·7장 |
| T-018 구독·제품 조회 API 계약 제안서 | 최소 GET·권한·오류·테스트 검토 | 2장·3장·4장 |
...
- [T-017A·T-022 검토 입력 패킷](../../../handoffs/20260801_t017a_t022_검토_입력_패킷.md)
- [T-017A 합성 사용자 계정 관리 설계서](../technical/backend/t017a_합성_사용자_계정_관리_설계서.md)
- [T-022 증상 제출 API 설계·계약 Gate](../technical/backend/t022_증상_제출_api_설계_및_계약_게이트.md)
- [T-018 구독·제품 조회 API 계약 제안서](../technical/backend/t018_구독_제품_조회_api_계약_제안서.md)
