...
- 교차 영역 최종 결정: 윤승혁(PM)
- 현재 판정: `BACKEND_AI_INTEGRATION_PENDING`

이 문서는 T-005 DB/API와 T-006 AI 계약 정합성 검토의 유효 내용,
이동윤 인계 재검수에서 확인한 기술 반례와 수락 조건을 하나로 통합한다.
담당자별 실행·반환 형식은
[백엔드 팀 검토 및 인계 체크리스트](../../team_handover/백엔드_팀_검토_및_인계_체크리스트.md)를
따른다.
...
| 경로·기능 | 주관 | 협업·소비 |
|---|---|---|
| `contracts/codes/**` | 최지용 | 이동윤·윤승혁 검토 |
| `contracts/api/**` | 최지용 | Web·Mobile·AI 소비자 검토 |
| `contracts/ai/**` | 이동윤 | 최지용 Backend Adapter 소비 |
| `ai/**` | 이동윤 | Backend Runtime 소비 |
| `backend/integrations/ai/**` | 최지용 | 이동윤 계약 검토 |
| `backend/apps/evidence/**` | 최지용 | 이동윤 Evidence 입력 |
| Data AI/RAG Schema·Catalog | 김은진 | 이동윤 부관할 |
| 상태 계약·통합 결정 | 윤승혁(PM) | 최지용·이동윤 |
...
[T-005 워터브리지 PostgreSQL 통합 검증 보고서](../backend/20260731_t005_워터브리지_postgresql_통합_검증_보고서.md)의
`32/32`를 우선한다.
...
| 담당 | 다음 행동 | 완료 증거 |
|---|---|---|
| 이동윤 | Parity·로그·취소·검색 후검증·Revision 보완 | AI Commit, 테스트, HTTP·로그·검색 증거 |
| 최지용 | AI 계약 수령 후 Backend Adapter·Evidence 검증 | Backend Commit, Mapper·오류·멱등·상태 E2E |
| 김은진 | 계약·Data·Resilience 통합 QA | QA Commit 또는 결과, 불일치 0 |
| 윤승혁(PM) | 공통 코드·잔여 위험·완료 Gate 승인 | Decision 기록과 병합된 `main` SHA |
...
- 다른 담당자의 개인 문서를 수정하거나 삭제하지 않는다.

## 12. 통합 원본 제목 매핑

| 기존 문서 | 기존 H2 | 통합 위치 |
|---|---|---|
| T-005 DB / T-006 AI 계약 정합성 검토 | 문서 책임·협업·검토 | 1장 |
| 동일 | 검토 원본 | 2장 |
| 동일 | 확정된 DB·API 기준 | 2장 |
| 동일 | 최신 정합성 결과 | 2장·4장 |
| 동일 | T-006 반영 기준 | 4장·8장 |
| 동일 | 검증 경계 | 9장 |
| 동일 | 인계 사항 | 10장 |
| 최지용 → 이동윤 인계 및 요청사항 | 반송 보완 회신 재검수 | 3~7장 |
| 동일 | 계약 ↔ Runtime 잔여 반례 | 4장 |
| 동일 | Timeout·Retry·구조화 로그 재검증 | 6장 |
| 동일 | 검색 안전성·pgvector 재검증 | 7장 |
| 동일 | AI 계약 PR·State·추적 요청 | 2장·4장 |
| 동일 | 재현 가능한 AI Runtime 인계 | 5장 |
| 동일 | RAG·Evidence·DB Wave 입력 | 7~8장 |
| 동일 | 반환할 완료 증거 | 4~10장과 팀 체크리스트 |
| 동일 | 공동 후속 진행사항 | 8~10장 |
| 동일 | 금지·주의사항 | 11장 |
...
- [테이블 사전](../../../../database/watercare_table_dictionary.md)
- [T-005 데이터 설계 기준선](../../../../database/t-005/README.md)
- [팀 검토 및 인계 체크리스트](../../team_handover/백엔드_팀_검토_및_인계_체크리스트.md)
