# T-017A 합성 사용자 계정 관리 OWNER 설계

> 기준일: 2026-07-31
> 작성·설계 책임: 최지용(Backend·Database)
> 협업·검토: 윤승혁(PM), 김은진(Data·QA·DevOps)
> 상태: `OWNER_DESIGN_READY_REVIEW_PENDING`
> 대상: `T-017A` 설계만
> 후속 구현: `T-017B` 2026-08-03, `T-017C` 2026-08-04~05
> 데이터 범위: 합성 사용자와 내부 시연 관리자만
> 실행 원칙: `설계 결정 → 검토 → 한 작업 구현 → 즉시 검증`

이 문서는 합성 사용자 계정 관리의 OWNER 설계 기준선이다. 현재
구현된 T-017 인증·권한 기능과 앞으로 구현할 Django Admin·계정
...
| 작업 | 현재 판정 | 근거와 의미 |
| --- | --- | --- |
| `T-017` 가상 로그인·JWT·RBAC | `OWNER_IMPLEMENTATION_READY` | 현재 [준비도 검사기](../../../../../backend/apps/accounts/readiness.py)에서 Auth Runtime 11파일과 Auth 테스트 38건 통과를 확인했다. 작성자 외 리뷰·PostgreSQL 완료 증거는 별도 Gate다. |
| `T-017A` 계정 관리 설계 | `OWNER_DESIGN_READY_REVIEW_PENDING` | 이 문서에 OWNER 결정안을 고정했다. WBS 완료 조건의 윤승혁·김은진 검토 증거는 아직 없다. |
| `T-017B` 내부 계정 관리 | `NOT_IMPLEMENTED` | 2026-08-03 구현 대상이다. 현재 [accounts Admin](../../../../../backend/apps/accounts/admin.py)은 설명 문자열만 있고 Admin 앱·Session·CSRF·URL·Custom UserAdmin이 구성되지 않았다. |
| `T-017C` 계정 수명주기·감사 | `NOT_IMPLEMENTED` | 2026-08-04~05 구현 대상이다. 비활성화 Service, 전체 Refresh 폐기, 재활성화 전 Token 세대 차단, 계정 전용 감사 원장이 없다. |

`T-017`의 `OWNER_IMPLEMENTATION_READY`를 `T-017A~C` 완료로 확장
...
윤승혁·김은진 리뷰에서 별도 필드 대신 다른 불변식이 승인되면
...
Model명·테이블명·JSON Schema·DB-level 불변식은 김은진의 Migration·QA
...
| ID | 단계 | 시나리오 | 기대 결과 |
| --- | --- | --- | --- |
| `A-DES-01` | T-017A | User 단일 원장·권한 4축·삭제·감사 정책 검토 | 윤승혁·김은진 결정과 이견이 기록됨 |
...
| 순서 | 담당 | 작업 | 반환 증거 |
| ---: | --- | --- | --- |
| 1 | 최지용 | T-017A OWNER 설계 후보와 현재 Auth 증거 고정 | Branch·Commit·이 문서·readiness JSON |
| 2 | 윤승혁 | 권한·접근·마지막 관리자·오류 코드 정책 검토 | 승인/변경 요청과 결정일 |
| 3 | 김은진 | Migration·합성 backfill·Token·감사·테스트 검토 | 재현 계획·위험·필수 QA 항목 |
| 4 | 최지용 | 반영된 T-017A를 기준으로 T-017B 한 작업씩 구현·검증 | Admin 표적 테스트·PostgreSQL·전체 회귀 |
| 5 | 김은진 | T-017B 비작성자 재현 | 빈 DB·기존 DB·Admin 보안 결과 |
| 6 | 최지용 | T-017C Lifecycle·감사를 한 작업씩 구현·검증 | Token·동시성·감사·rollback 증거 |
| 7 | 김은진 | T-017C와 T-047A 후보 독립 QA | 테스트 Matrix 결과·잔여 위험 |
| 8 | 윤승혁 | 리뷰 증거 확인 후 `main` 병합 | 병합된 40자리 `main` SHA |

한 단계가 실패하면 다음 단계로 넘어가지 않는다. 팀원은 최지용의
작업 Branch를 임의 기준선으로 사용하지 않고 PM이 병합한 `main` SHA를
공용 기준으로 사용한다.
...
- [ ] 윤승혁 정책 검토 증거
- [ ] 김은진 Migration·QA 검토 증거
- [ ] WBS의 T-017A 승인·상태 갱신

따라서 현재 최종 판정은 `OWNER_DESIGN_READY_REVIEW_PENDING`이다.
