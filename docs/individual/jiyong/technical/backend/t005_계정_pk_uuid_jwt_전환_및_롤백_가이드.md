...
> 현재 상태: `IMPLEMENTED_AND_AUTHOR_POSTGRESQL_VERIFIED`
> 독립 완료 Gate: 비작성자의 fresh·populated PostgreSQL 재현과 PM 리뷰
> 적용 원칙: `작업 → 즉시 검증 → 다음 작업`

## 1. 결론과 범위

이번 Gate는 Accounts의 과도기 문자열 기본키를 내부 자동 증가 정수
...
| 우선순위 | 담당 | 재현 | 필수 증거 |
| ---: | --- | --- | --- |
| P0 | 김은진 또는 검증 담당 | 빈 격리 PostgreSQL 전체 Migration | 명령 Exit code, 0003 PASS, 최종 `bigint` 타입 |
| P0 | 김은진 또는 검증 담당 | pre-0003 문자열 PK·Inquiry FK·M2M 행을 넣은 populated 전환 | legacy ID·FK·M2M 보존 결과, 신규 sequence |
| P0 | 윤승혁(PM) | Migration·JWT breaking change 리뷰 | 승인 PR 또는 결정 기록 |
| P1 | Web·Mobile 담당 | legacy Token 401 처리와 재로그인 | Refresh loop 0, 로그인 복귀 확인 |

독립 populated 검증에서는 current Model을 pre-0003 Schema에 직접
...
| 대상 | 전달 내용 | 다음 행동 | 완료 증거 |
| --- | --- | --- | --- |
| 김은진 | 0003 알고리즘, FK·M2M 목록, fresh·populated 검증 기준 | 독립 PostgreSQL 재현과 전체 QA | 실행 로그·타입·보존 결과 |
| 윤승혁(PM) | 내부 PK breaking change, 비가역 Rollback, JWT 세션 만료 | 리뷰·병합·배포 시점 결정 | 승인 기록과 기준 SHA |
| 한예나 | Web legacy Token 401 처리 | Token 제거 후 로그인 복귀 | Refresh loop 없는 화면 검증 |
| 양정현 | Mobile legacy Token 401 처리 | Token 제거 후 로그인 복귀 | 재로그인 동작 검증 |
