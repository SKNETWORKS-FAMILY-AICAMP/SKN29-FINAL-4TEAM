...
### 8.2 2026-07-31 WaterBridge 현행 실행 기준

위 8.1은 2026-07-29 `watercare` 실행 이력이다. 현재 기본 PostgreSQL
데이터베이스는 `waterbridge`, Schema는 `public`이다. Docker Compose
프로젝트명과 `watercare-postgres-data` Volume은 DB 이름이 아니므로
기존 데이터를 보존하기 위해 그대로 유지한다.

저장소 루트에서 다음 명령으로 현재 환경을 확인한다.

```powershell
python --version
python .\scripts\development\check_environment.py --service backend
python .\scripts\development\check_environment.py --service backend --postgresql
python .\scripts\development\check_environment.py --service backend --full --postgresql
```

2026-07-31 작성자 PC에서 위 전체 Gate는 Python 3.13.13, pip 26.0.1,
constraints 32개, 추가 패키지 0개, requirements fingerprint 일치,
PostgreSQL 16.14, `waterbridge/public`, Migration 누락·drift 0,
SQLite 740 passed·PostgreSQL 전용 11 skipped로 통과했다. 별도
PostgreSQL 전체 회귀는 751 passed다.

T-005 계약 테이블 32개는 모두 물리적으로 유지한다. Active 데이터
범위는 13개이고 Target-only 19개는 현재 0행이다. 이는 별도 Schema나
축소 Migration이 아니다. 합성 Importer는 기본 `waterbridge`와 Legacy
`watercare`에서 `--dry-run`까지 실행하지 않고 새 빈 격리 DB에서만
검증한다.

DB 전환·복구, Seed 2회, 격리 Importer Replay와 전체 회귀의 상세 근거는
...
| --- | --- | --- | --- | --- |
| 김은진 | `scripts/development/**`, requirements·constraints, 전체 검사 | 새 Pull 환경에서 bootstrap과 Migration 적용 후 `--full --postgresql`, Health·Auth Smoke를 재현하고 Windows/Linux 차이를 기록 | 사용한 Python, 명령, exit code, 테스트 수를 PR 또는 Issue에 기록 | 작성자 현재 SQLite 740·PostgreSQL 751·PostgreSQL 읽기 전용 Gate 검증 완료, 독립 재현 미확인 |
