# 합성 데이터 스키마·적재기·PostgreSQL 검증 가이드

- 최초 구현·검증일: 2026-07-29
- 문서 통합일: 2026-07-31
- 담당 영역: Backend·Database
- 협업 대상: Data/QA, PM, Backend API 소비자
- 현재 판정: `DB_FULL_VERIFIED` — 빈 격리 PostgreSQL의 합성 데이터 367건 범위
- 제외 범위: 실제 고객·운영 데이터, 기본 개발 DB에 대한 합성 데이터 Import

이 문서는 합성 데이터용 도메인 Model·Migration, 정식 적재기,
PostgreSQL 실행 검증을 하나의 재현 기준으로 제공한다. 다음 세 문서의
유효 내용을 통합했으며, 단계별 수치와 현재 판정을 혼동하지 않도록
역사 기록과 최종 실측을 분리했다.
...
판정은
[T-005 워터브리지 PostgreSQL 통합 검증 보고서](20260731_t005_워터브리지_postgresql_통합_검증_보고서.md)를
우선한다.
...
| Data/QA | Fixture 12종, Crosswalk, Schema, Hash, 결정성 | Data Owner Review, QA 결과, Manifest Hash |
| PM·Workflow | 문의·방문 상태와 125개 전이·감사 | 승인 계약 버전과 불일치 0 |
| 소비자 QA | 적재 데이터의 API 조회·권한·오류 경계 | 역할별 API Smoke와 E2E 결과 |

관련 문서:

- [합성 데이터 픽스처·해시·교차표 검증 보고서](../contracts/합성_데이터_픽스처_해시_교차표_검증_보고서.md)
- [합성 고객 데모 로그인 가이드](../../manuals/합성_고객_데모_로그인_가이드.md)
...
| 기존 문서 | 기존 H2 | 통합 위치 |
|---|---|---|
| 합성 데이터 도메인 스키마·Migration 개발 인계서 | 작업 목적과 완료 범위 | 1장 |
| 동일 | 구현 산출물 | 2.1절 |
| 동일 | 설계상 연쇄 오류 방지 장치 | 2.2절 |
| 동일 | Migration 체인 | 2.3절 |
| 동일 | 검증 결과 | 7장 |
| 동일 | 의도적으로 변경하지 않은 기준 자산 | 6장 |
| 동일 | 인계 및 후속 작업 | 8장 |
| 합성 데이터 정식 적재기 개발·인계서 | 목적과 완료 범위 | 1장·3장 |
| 동일 | 구현 산출물 | 3.1절 |
| 동일 | 처리 순서와 트랜잭션 경계 | 3.2절 |
| 동일 | 프로필과 완료 기준 | 1장·4.2절 |
| 동일 | 식별자·갱신·시간 정책 | 3.3절 |
| 동일 | 적재 원장과 provenance | 3.4절 |
| 동일 | 실행 가이드 | 5장 |
| 동일 | 검증 결과 | 4장·7장 |
| 동일 | 협업 인계 | 8장 |
| PostgreSQL 합성 Handoff Runtime 검증·인계서 | 최종 판정 | 1장 |
| 동일 | 근거의 단일 원본 | 3.1절·8장 |
| 동일 | 프로필별 실측 | 4.2~4.3절 |
| 동일 | 상태·감사 정합성 | 4.4절 |
| 동일 | 합성 고객 Auth Runtime | 별도 데모 로그인 가이드 |
| 동일 | 재현 명령 | 5장 |
| 동일 | 안전 주의사항 | 6장 |
| 동일 | 완료 범위와 남은 범위 | 1장·7장 |
| 동일 | 팀 인계와 반환 순서 | 8장 |
| 동일 | 기본 개발 DB 후속 Migration·Seed 실측 | 2.3절·6장 |
