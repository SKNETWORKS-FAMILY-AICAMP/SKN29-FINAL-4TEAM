...
포함하지 않고 [워터브리지 백엔드 설치·Migration·Seed·복구 가이드](<../../manuals/워터브리지_백엔드_설치_마이그레이션_시드_복구_가이드.md>)의
명시적 순서로 실행한다.
...
상세 적용·보정 증거는
[합성 데이터 스키마·적재기·PostgreSQL 검증 가이드](합성_데이터_스키마_적재기_postgresql_검증_가이드.md)에
누적했다.
