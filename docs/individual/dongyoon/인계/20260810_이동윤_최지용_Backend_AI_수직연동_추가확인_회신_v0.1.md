...
| 요청 | 결과 | 검증 근거 |
|---|---|---|
| AI 환경 재현 | 완료 | Python 3.13.13, pip check PASS, 127 passed |
...
```text
ai_unit_tests=127 passed, 3 warnings
ai_unit_tests_history=103은 F01~F12 Fixture 추가 전 중간값; 115는 Fixture 12개 포함 계약 2.0.0 당시 최종값; 121은 계약 3.0.0 안전 ID·canonical evidence·runtime identity 검증 추가값; 126은 상담 요약 기준선 4건과 자연어 누수 안전 회귀 1건 추가값; 현재 127은 pyproject·requirements.txt·requirements.lock 직접 의존성 정합성 회귀 1건을 추가한 최신값
pip_check=PASS
