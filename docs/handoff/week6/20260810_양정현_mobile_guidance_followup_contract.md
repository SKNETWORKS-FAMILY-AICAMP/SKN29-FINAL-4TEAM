...
- 기준 commit: $jeongHead
- 포함된 latest main: $mainHead
- 담당 범위: Mobile
- 상태: **CONTRACT_READY / BACKEND_ROUTE_PENDING**
...
현재 Mobile GuidanceData는 아래 고객 표시 계약을 기대한다.
...
Backend DB의 Guidance row와 Mobile GuidanceData는 1:1 동일 모델이 아니다.
...
## 4. PROPOSED — Guidance 조회 계약

> 아래 URL/operation 이름은 **제안안**이며 현재 실제 Backend route가 아니다.
> Backend 담당자가 route 이름을 확정하면 Mobile이 그 계약에 맞춰 연결한다.
...
- 다른 고객 Inquiry 조회 시 403 또는 프로젝트 표준 ownership error
...
권장 방식 중 하나를 Backend가 확정:

- HTTP 202 + 명시적 processing payload
- 또는 프로젝트 표준 상태 응답
...
- 위험/근거없음/알 수 없는 코드에서는 PENDING_CONSULTATION으로 fail-closed
...
- [ ] Galaxy real Backend instrumentation
...
3. WaterCareApi에 **실제 route만** 추가
4. DTO 작성
5. Remote repository 연결
6. Fixture fallback을 실제 결과처럼 사용하지 않음
7. Unit Test
8. Backend REST Smoke
9. Galaxy instrumentation
10. Week5 Runtime Matrix 갱신
...
**CONTRACT_READY / BACKEND_ROUTE_PENDING**
...
Mobile은 3.4 route가 실제 등록되기 전까지 존재하지 않는 API를 구현하지 않는다.