...
8. `latest_schema_report.json`, `latest_integrity_report.json`, `latest_quality_report.json`에는 다음을 기록한다.
    ...
    - 실패 레코드 식별자 또는 확인 경로
    - 실행 명령과 종료 코드
9. 전처리 전후 실제 건수와 제외 사유를 계산한다. 기존 보고서의 PDF 2파일·96쪽, FAQ 119건, 선별 후보 20건, MVP RAG 7건, 확장 RAG 4건, 합성 시나리오 6건은 현재 파일을 다시 계산한 뒤 일치할 때만 사용한다.
...
docs/testing/
└─ week3-rag-evaluation-criteria.md
```

### 세부 작업 지침

1. 평가 Case는 최소 다음 필드를 포함한다.
    - `case_id`
    - `scenario_id`
    - `query`
    - `exact_sales_code`
    ...
    - `top_k: 5`
    - `pass_condition`
2. 대표 저출수 질의는 `SYN-JAC104-002` 또는 확정된 Case ID로 고정하고, `WPUJAC104DWH`, D세대, 공식 매뉴얼 REV.00 38쪽을 정답 근거로 설정한다.
...
5. 필수 평가 기준은 다음과 같이 적용한다.
    - 대표 P0 질의마다 정답 문서·페이지가 Top-5에 포함되어야 한다.
    - 대표 P0 질의 중 하나라도 실패하면 검색 평가를 통과로 판정하지 않는다.
    - 금지 문서와 다른 모델·세대의 근거가 최종 정답으로 포함되면 실패로 처리한다.
    - 검색 결과의 모델·세대·허용 정책이 질의와 일치해야 한다.
    - 근거가 없을 때 빈 결과 또는 안전한 Fallback으로 이어져야 한다.
6. Recall@5와 Hit@5는 보조 지표로 기록할 수 있지만, 3주차 필수 합격선으로 사용하지 않는다.
7. 이동윤이 생성한 검색 결과에는 `case_id`, 실행 설정, `BAAI/bge-m3` 원본 출력 차원, Index Manifest, Top-5, 필터 적용 여부와 결과 순위를 남기게 한다.
8. MVP 검색은 별도의 HNSW·IVFFlat Index 없이 Exact Search를 기준으로 평가한다.
9. 김은진은 정답표와 실제 결과를 비교하고 통과·실패·수동 검토 필요를 구분한다.
10. 검색 점수만 높고 모델·세대·문서 정책이 틀린 경우 실패로 처리한다.
11. 평가 결과에 실패 Case가 있으면 데이터 오류, 필터 오류, Vector 적재 오류, 질의 구성 오류 중 추정 원인과 담당자를 기록한다.

### 완료 기준

- 대표 P0 질의 전부에서 정답 문서·페이지가 Top-5에 포함된다.
- JAC104 S세대·IAC425·IAC506·미검증 FAQ의 혼입 차단 Case가 존재한다.
- 다른 모델·세대의 근거가 정답으로 포함되는 오염은 0건이다.
- 근거 없음·모델 불일치 Case의 기대 결과가 명시되어 있다.
- 평가 세트와 실제 검색 결과가 동일한 `case_id`로 연결된다.
- `BAAI/bge-m3` 원본 출력 차원과 DB VectorField 차원이 일치한다.
- MVP 검색이 Exact Search로 수행되었는지 확인할 수 있다.
- 평가 입력·설정·Vector 데이터 버전·결과가 반복 실행 가능하게 기록된다.
- 검색 실패의 원인이 데이터·필터·Vector 적재·질의 중 어디에 있는지 추적 가능하다.

### 산출물

- 대표 증상별 Top-5 검색 평가 데이터
...
1. `data/synthetic/fixtures/**`를 Web·Mobile·Backend·AI가 사용하는 합성 업무 데이터의 유일한 기준본으로 관리한다.
2. 역할별 가상 사용자를 고객·상담사·방문기사·운영 담당자로 구분하고 역할 코드·활성 상태·가상 표시명을 저장한다.
3. `products.json`에는 MVP 제품과 확장 제품을 구분한다.
    - `WPUJAC104DWH`: MVP 기본 제품
    - `WPUIAC425SNW`: 확장 전용, 기본 화면·검색에서 제외
    - IAC506: 활성 Fixture에 포함하지 않음
4. `subscriptions.json`은 사용자·제품·사용 시작일·관리 유형·구독 상태를 참조 무결성이 있는 ID로 연결한다.
5. `care_histories.json`에는 필터 교체·살균·세척 등 대표 관리 이력과 공식 기준 또는 시연 규칙에 따른 다음 케어 예정일을 포함한다.
6. 일시 값은 한국 시간대가 포함된 ISO 8601 형식으로 작성한다.
    - 예: `2026-07-27T17:30:00+09:00`
    - Backend 적재 검증에서는 DB에 UTC로 변환되어 저장되는지 확인한다.
    - 시간대가 없는 일시 문자열은 사용하지 않는다.
7. 대표 문의 6건은 다음 업무 유형을 포함하도록 확인한다.
    - 자가 해결
    - 상담 인계·상담 후 처리
    - 방문 인계·방문 후 처리
    - 위험 감지·사용 제한
...
8. 각 시나리오에 다음 기대값을 연결한다.
    ...
    - `allowed_actions`
    - 최종 해결 여부
9. `usage_guidance_status`는 다음 네 값만 사용한다.
    - `NORMAL`
    - `PARTIAL_STOP`
    - `TOTAL_STOP`
    - `PENDING_CONSULTATION`
10. 주요 리소스에는 다음 식별자를 구분하여 사용한다.
- `public_id`: 외부 API에서 사용하는 UUID
- `inquiry_code`, `scenario_code`: 사람이 확인하는 업무·시연 식별자
- 내부 정수 PK: Backend가 DB 적재 시 자동 생성
1. `DEMO-INQ-002`는 문의 업무 코드이며 DB의 실제 PK로 사용하지 않는다. Fixture에는 DB 자동 증가 정수 PK를 고정값으로 저장하지 않는다.
2. `SYN-JAC104-002`와 `DEMO-INQ-002`는 대표 저출수 E2E 기준으로 고정하고, 다른 파일에서도 동일한 코드와 공개 식별자를 사용한다.
3. Fixture의 `public_id`는 중복되지 않아야 하며 다른 파일의 참조가 모두 존재해야 한다.
4. Seed는 반복 실행해도 중복 행이 증가하지 않도록 공개 UUID 또는 업무 코드를 기준으로 Upsert한다.
5. `build_service_fixtures.py`로 Backend·AI·Web·Mobile용 Fixture를 자동 생성한다. 생성된 서비스별 Fixture를 직접 수정하지 않고, 변경이 필요하면 `data/synthetic/fixtures/**`의 기준본을 수정한 뒤 다시 생성한다.
6. `validate_synthetic_data.py`에서 Schema, 공개 ID 중복, 참조 무결성, 상태·코드 유효성, Evidence 참조, MVP 범위와 개인정보 패턴을 검사한다.

### 완료 기준

- `data/synthetic/fixtures/**`가 합성 업무 데이터의 유일한 기준본으로 관리된다.
- 역할별 계정과 제품·구독·케어 데이터가 DB에 적재 가능한 구조이다.
- 대표 시나리오 6건이 서로 다른 업무 흐름과 기대 결과를 가진다.
- 내부 정수 PK, 공개 UUID, 업무·시연 코드가 구분되어 있다.
- 모든 참조·상태·근거 연결이 유효하다.
- 사용 안내 상태가 확정된 네 개의 코드로 통일되어 있다.
- API 기대 일시는 `+09:00` ISO 8601 형식을 사용하고 DB에는 UTC로 저장된다.
- 동일 Seed를 반복 실행해도 중복 데이터가 생성되지 않는다.
- 실제 개인정보·운영 연락처·실제 상담 기록이 포함되어 있지 않다.
- `SYN-JAC104-002`, `DEMO-INQ-002`, 공식 38쪽 근거가 일관되게 연결된다.
- Web·Mobile·Backend·AI용 Fixture를 기준본에서 재생성할 수 있다.

### 산출물

- 역할별 가상 계정·제품·구독·케어 Fixture 기준본
- 대표 합성 문의 6건 기준본
- 상태·근거·안전·인계 기대 결과
- Synthetic JSON Schema
- 서비스별 Fixture 자동 변환 스크립트와 생성 결과
- Seed 적재·검증 자료
...
docs/testing/
├─ week3-p0-acceptance-criteria.md
├─ test-case-matrix.md
└─ results/**
```
...
3. 3주차 최소 인수 Case를 다음과 같이 구성한다.
    - Access Token과 Refresh Token 로그인 성공·실패
    - Access Token 60분 만료
    - Refresh Token 7일 고정 만료
    - Access Token 만료 후 Refresh Token을 이용한 재발급
    - 로그아웃 후 서버에서 폐기된 Refresh Token의 재사용 차단
    - 역할상 허용되지 않은 기능 접근 시 403
    - 다른 사용자가 소유하거나 담당하는 개별 리소스 접근 시 404
    - 고객 본인 데이터만 조회
    - 기사 미배정 건 접근 차단
    - 문의 생성과 같은 `inquiry_id` 입력 누적
    - 필수값 누락·미지원 제품·차단 모델
    - Backend↔AI 정상 Schema와 필수 필드 누락
    - `NORMAL`, `PARTIAL_STOP`, `TOTAL_STOP`, `PENDING_CONSULTATION` 사용 안내
    - State Machine 허용·금지 행동
    - 행동별 상태 변경 Endpoint 호출
    - 동일 `idempotency_key` 재전송
    - 잘못된 `state_version` 요청 시 409
    - 409 응답에 최신 상태·`state_version`·`allowed_actions` 포함
    - Backend↔AI 전체 Timeout 30초
    - AI 내부 최대 재시도 1회
    - Backend 자동 재시도 0회
    - 내부 정수 PK·공개 UUID·업무 코드 분리
    - `BAAI/bge-m3` 원본 출력 차원과 DB VectorField 차원 일치
    - Exact Search 기반 Top-5 검색
    - EvidenceCard의 내부 경로·원문 전체·검색 점수 비노출
4. 실제 기능이 아직 미구현된 Case는 `blocked`, `not_implemented`, `fixture_ready`로 구분하고 실패를 숨기지 않는다.
5. API Contract Fixture는 최지용의 구현 코드를 복제하지 않고 `contracts/api/**`, `contracts/error-codes/**`, `contracts/codes/**`의 병합된 계약 파일을 기준으로 검증한다.
6. AI Contract Fixture는 `contracts/ai/**`와 이동윤의 Pydantic 모델이 동일한 Enum·필수 필드를 사용하는지 확인한다.
7. State Machine 테스트는 `contracts/state-machine/**`의 상태·이벤트·가드·역할별 `allowed_actions`를 기준으로 한다.
8. 설명 문서와 `contracts/**`의 내용이 다르면 `contracts/**`를 테스트 기준으로 사용한다.
9. 계약 변경 PR이 관련 구현 PR보다 먼저 병합되었는지 확인한다.
10. 오류 Fixture는 400·401·403·404·409·422·5xx·Timeout·파싱 실패를 구분한다.
11. Data CI에는 다음 검사를 우선 포함한다.
- `data/raw/` 원본 Git 포함 여부
- JSON·JSONL·CSV 구문
- JSON Schema 일치
- 공개 ID·페이지·문서·근거 참조 무결성
- MVP·확장 데이터 혼입
- IAC506 활성 데이터 존재 여부
- 기준 Fixture와 서비스별 생성 Fixture의 일치 여부
1. Contracts CI는 계약 파일이 실제로 존재하고 검증 명령이 확정된 항목부터 시작한다. 비어 있는 Workflow를 성공으로 표시하지 않는다.
2. CI에서 원본 PDF·LLM API·외부 Vector Store가 없어도 기본 데이터·Schema 검증이 가능하도록 Mock 또는 저장소 내 구조화 데이터만 사용한다.
3. 각 개발 담당자에게 7월 29일까지 제공해야 할 최소 증빙 형식을 전달한다.
- 실행 명령
- 정상 결과
- 오류·예외 결과
- 사용 Fixture
...
- 인증 60분·7일 정책과 로그아웃 후 Refresh Token 폐기가 검증 항목에 포함되어 있다.
- 역할 권한 부족 403과 타 사용자 리소스 404가 구분되어 있다.
- 행동별 상태 변경 Endpoint와 409 응답 필드가 검증 항목에 포함되어 있다.
- Backend 전체 Timeout 30초, AI 내부 재시도 1회, Backend 재시도 0회가 검증된다.
- 정수 내부 PK·공개 UUID·업무 코드의 분리가 검증된다.
...
7. RAG 후속 활용에는 실제 청크 파일, `BAAI/bge-m3` 원본 출력 차원, Vector Store, Exact Search 설정, 인덱싱 대상·제외 건수, 대표 Top-5 검색 결과와 재현 명령을 이동윤에게 받아 반영한다.
...
9. 데이터베이스·저장소 설계 문서에는 김은진 담당 영역을 다음과 같이 작성한다.
    ...
    - `data/synthetic/fixtures/**` 기준본과 서비스별 Fixture 변환 원칙
    - Seed·Fixture 적재와 초기화 원칙
    - 내부 정수 PK·공개 UUID·업무 코드의 구분
    - DB UTC 저장과 API `+09:00` ISO 8601 표현 기준
    - 문자열 칼럼과 Django `TextChoices` 기반 고정 코드 관리
...
11. Vector Store 설명에는 업무 DB와 벡터 저장소의 책임을 구분하고, 청크 ID·문서 ID·페이지·모델·세대·사용 정책을 연결하는 방법과 Exact Search 적용 범위를 기록한다.
12. API·AI·State Machine·공통 코드 설명은 병합된 `contracts/**`를 기준으로 작성한다.
13. 두 문서의 수치·파일 경로·필드명이 실제 저장소와 일치하는지 교차 확인한다.
14. 7월 29일까지 본문이 모두 작성되고 실제 증빙 링크가 연결된 검토본을 윤승혁에게 전달한다.

### 완료 기준

- 데이터 전처리 결과서가 실제 프로젝트 데이터·수치·경로·도구로 작성되어 있다.
- 템플릿의 다른 기수·다른 팀·예시 데이터가 남아 있지 않다.
- 데이터 전처리 결과서의 수치가 Manifest·품질 리포트·Top-5 검색 평가 결과와 일치한다.
- 대표 P0 질의가 모두 Top-5 기준을 통과했는지 문서에서 확인할 수 있다.
- 데이터베이스·저장소 설계 문서의 데이터 분류·Seed·품질·보안 항목이 실제 구조와 일치한다.
- 내부 정수 PK·공개 UUID·업무 코드의 구분이 문서와 Fixture에서 일치한다.
- DB UTC 저장과 API 한국 시간대 표현 기준이 문서에 반영되어 있다.
- 관계형 DB·Vector Store·파일 저장소의 책임과 연결 키가 구분되어 있다.
- `data/synthetic/fixtures/**`에서 서비스별 Fixture를 재생성할 수 있다.
...
필수 업무, 7월 29일 산출물 작성과 팀 검토 대응이 끝난 뒤에만 착수한다. 아래 업무는 공통 협의 결과 Q-01~Q-10과 contracts/**에서 확정된 Schema·상태·Fixture만으로 비교적 독립적으로 진행할 수 있는 후속 작업을 선정한 것이다.
...
- [ ]  대표 합성 시나리오 6건과 상태·안전·근거·인계 기대 결과가 연결되어 있다.
