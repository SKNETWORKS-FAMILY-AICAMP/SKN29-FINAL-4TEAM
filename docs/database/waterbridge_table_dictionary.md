# WaterBridge Table Dictionary

> 문서 상태: **HISTORICAL_SNAPSHOT — v0.5 설계 초안 보존본**
> 설계 Snapshot: `v0.5` / 기준일: 2026-07-27
> 범위: 32개 설계 테이블 · 526개 필드 · 85개 물리 FK · 57개 논리 코드 참조
> 현행 우선 원천: [ADR 인덱스](../adr/README.md),
> [T-005 기준 안내](t-005/README.md)와
> [T-005 Physical Contract v1.3](t-005/t005_physical_contract_v1.3.json)
...
- `팀 결정 필요`: 당시 초안에서 미결 후보를 표시한 **역사 주석**이다.
  현재 구현 착수의 자동 차단 조건이 아니다. 현재 값은 ADR 0008~0011과
  Physical Contract v1.3을 우선 적용하고, 외부 담당 입력이 필요한 항목만
  해당 담당 계약에서 후속 정합화한다.
- 타입·기본값·제약은 과거 설계값이다. 현행 운영 스키마 판정은 활성
  Physical Contract, Django Migration과 실제 PostgreSQL 검증 증거를
  따른다.
