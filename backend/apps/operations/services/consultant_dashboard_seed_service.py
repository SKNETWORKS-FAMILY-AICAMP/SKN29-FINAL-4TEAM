...
NOTICE_FIXTURES = (
    (
        ...
        "긴급 문의 응대 절차 안내",
        (
            "누수·감전·이상 냄새 등 안전 위험 문의는 일반 문의보다 "
            "먼저 처리합니다.\n\n"
            "1. 고객에게 제품 사용을 즉시 중지하고 안전한 거리에서 "
            "상태를 확인하도록 안내해 주세요.\n"
            "2. 제품 코드, 발생 시점, 위험 신호와 이미 시행한 조치를 "
            "확인하되 임의 분해나 수리를 요구하지 마세요.\n"
            "3. 확인한 내용과 고객에게 전달한 안전 안내를 상담 기록에 "
            "남기고 긴급 상담 절차로 연결해 주세요.\n\n"
            "위험 신호가 남아 있으면 자가조치만으로 상담을 종료하지 "
            "말고 고객케어팀에 즉시 공유해 주세요."
        ),
        "고객케어팀",
        date(2026, 8, 18),
    ),
    (
        ...
        (
            ...
            "\n\n"
            "조사 참여는 고객의 자율 선택이며 응답 여부나 점수는 상담 "
            "처리 결과, 재상담 또는 방문 지원과 연결하지 않습니다. "
            "고객이 참여를 원하지 않으면 추가 권유 없이 상담을 "
            "마무리해 주세요.\n\n"
            "조사 화면에는 상담 해결 여부와 서비스 경험만 입력하도록 "
            "안내하고 비밀번호, 인증번호, 결제정보와 같은 민감정보는 "
            "작성하지 않도록 알려 주세요."
        ),
        "고객경험팀",
        date(2026, 8, 18),
    ...
    (
        ...
        "상담 시스템 정기 점검 안내",
        (
            "정기 점검 시간에는 신규 문의 저장과 상태 갱신이 지연될 "
            "수 있습니다.\n\n"
            "작업 전 문의번호, 현재 상태, 상관 ID와 작성 중인 상담 "
            "내용을 확인해 주세요. 저장 오류가 발생하면 같은 버튼을 "
            "반복해서 누르지 말고 입력 내용을 보존한 뒤 최신 문의 "
            "상태를 다시 조회해 주세요.\n\n"
            "점검이 끝난 뒤에는 처리 중이던 문의의 저장 결과와 상태 "
            "이력을 대조하고, 불일치가 있으면 시스템운영팀에 문의번호와 "
            "발생 시각을 전달해 주세요."
        ),
        "시스템운영팀",
        date(2026, 8, 17),
    ...
    (
        ...
        "8월 상담 근무 일정 확인 요청",
        (
            "팀별 근무표와 긴급 문의 당번을 승인된 일정표에서 확인해 "
            "주세요.\n\n"
            "근무 시작 전 담당 시간대, 인수인계 대상 문의와 긴급 문의 "
            "연락 순서를 확인합니다. 일정 변경이 필요하면 개인 간 교환만 "
            "진행하지 말고 변경 사유와 대체 담당자를 고객케어팀에 먼저 "
            "알려 주세요.\n\n"
            "확정된 변경사항은 팀 공용 일정표에 반영된 뒤부터 적용하며, "
            "고객 개인정보는 일정표나 인수인계 메모에 기재하지 마세요."
        ),
        "고객케어팀",
        date(2026, 8, 16),
    ),
    (
        ...
        "임직원 건강검진 신청 안내",
        (
            "건강검진 대상자는 승인된 사내 복지 절차를 통해 검진 일정을 "
            "신청해 주세요.\n\n"
            "신청 전에 대상 여부와 가능한 기간을 확인하고, 근무 일정과 "
            "겹치면 팀 담당자와 사전에 조정합니다. 일정 변경이나 취소가 "
            "필요한 경우에도 동일한 신청 경로를 사용해 주세요.\n\n"
            "검진 결과나 건강 상태와 같은 민감정보는 상담 시스템, 공용 "
            "메신저 또는 근무 일정표에 기록하지 마세요."
        ),
        "경영지원팀",
        date(2026, 8, 15),
    ),
    (
        ...
        "정수기 안전 점검 상담 교육",
        (
            "제품별 공식 매뉴얼과 안전 점검 문구를 확인한 뒤 상담을 "
            "진행해 주세요.\n\n"
            "고객 제품 코드와 같은 모델의 근거만 사용하고, 발생 시점과 "
            "증상 조건처럼 고객이 안전하게 확인할 수 있는 항목만 "
            "질문합니다. 확정 진단, 임의 분해 또는 다른 제품의 조작법을 "
            "안내하지 마세요.\n\n"
            "누수·감전·연기 등 명확한 위험 신호는 제품 사용 중단과 상담 "
            "연결을 우선하고, 방문 전환 여부와 고객 안내 내용은 상담 "
            "기록에 남겨 주세요."
        ),
        "품질관리팀",
        date(2026, 8, 14),
