...
if command -v curl >/dev/null 2>&1; then curl -L --fail "$URL" -o "$DEST"; elif command -v wget >/dev/null 2>&1; then wget -O "$DEST" "$URL"; else echo "curl 또는 wget이 필요합니다." >&2; exit 1; fi
