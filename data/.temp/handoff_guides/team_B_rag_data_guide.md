...
6. `data/processed/structured/jac104_evidence_registry.jsonl`
...
- `model_family_direct`: 조건부 보조만 가능, D 세대 매뉴얼 우선, FAQ 단독 답변 금지
...
WPU-IAC425 청크 4건은 `data/processed/structured/rag_expansion_iac425.jsonl`에 물리적으로 분리돼 있다. PM이 확장 시작을 승인하기 전 파일 자체를 기본 MVP 적재 대상에 포함하지 않는다.
...
- `exact_sales_code`, `source_landing_url`, `source_direct_download_url`
...
- `WPUJAC104S*` 자료와 `DEMO_METADATA`가 공식 검색 근거에 포함되지 않음
