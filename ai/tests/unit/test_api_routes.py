...
CORRELATION_ID = "018f2f9b-7c30-7981-b541-1a987c88b401"
ALTERNATE_CORRELATION_ID = "018f2f9b-7c30-7981-b541-1a987c88b402"
...
def test_local_mode_vector_failure_retries_once_then_returns_503(client, monkeypatch, caplog):
    ...
    payloads = [
        ...
        if f'"correlation_id": "{CORRELATION_ID}"' in record.message
    ]
...
def test_analyze_endpoint_rejects_correlation_id_mismatch(client):
    ...
    response = client.post(
        ...
        json=payload,
        headers={"X-Correlation-ID": ALTERNATE_CORRELATION_ID},
    )
...
def test_analyze_endpoint_rejects_non_uuid_correlation_without_echo(client):
    response = client.post("/api/v1/ai/analyze?mode=mock", json={
        "inquiry_id": INQUIRY_ID,
        "correlation_id": "not-a-uuid",
        "ai_request_id": "ai-req-invalid-correlation",
        "state_version": 1,
        "raw_symptom": "물이 나오지 않습니다.",
        "model_code": "WPUJAC104DWH",
    })
...
def test_validation_error_emits_single_safe_failure_log(client, caplog):
    ...
    assert payloads[0]["correlation_id"] == CORRELATION_ID
