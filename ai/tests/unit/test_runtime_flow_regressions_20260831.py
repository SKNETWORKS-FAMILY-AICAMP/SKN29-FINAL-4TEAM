...
def test_valid_safety_evidence_survives_structuring_validation():
    ...
    assert {item.signal_name for item in structurer.last_safety_signals.evidence} == {
        "electrical_component_damage", "exposed_wire",
    }
...
def test_rejected_structured_candidate_preserves_independent_safety_signals():
    raw = "정수기 전선 피복이 벗겨졌어요"
    structurer = SymptomStructurer(
        llm_client=_SemanticSymptomClient(
    ...
    assert structurer.last_safety_signals.requires_danger_policy is True
    assert RiskClassifier().classify(raw, safety_signals=structurer.last_safety_signals).risk_level == RiskLevel.DANGER
...
def test_hot_water_scenario_selector_removes_unmentioned_subscenarios():
    ...
    assert "화상에 주의" in selected
    assert "음용하지" not in selected  # This warning belongs to the no-output branch.
