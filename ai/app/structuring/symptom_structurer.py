...
from ..safety.signal_detector import detect_safety_evidence, supports_safety_quote
from .llm_contracts import SafetySignalEvidence
...
class SymptomStructurer:
    ...
    def structure(
        ...
        *,
        ...
        self.last_safety_signals = self._deterministic_safety_signals(raw_text, previous)
        ...
        with _STRUCTURING_TRACER.start_as_current_span(
            ...
            try:
                ...
                self.last_safety_signals = self._merge_safety_signals(
                    self.last_safety_signals,
                    self._validated_safety_signals(
                        response.safety_signals, raw_text=raw_text,
    ...
    def _structure_with_rules(
        ...
        return StructuredSymptom(
            ...
            target_water_type=(
                self.normalizer.normalize_water_type(raw_text)
                or answer_by_field.get("target_water_type")
            ),
        ...
        return (
            candidate.model_copy(
                update={
                    ...
                    "target_water_type": (
                        self.normalizer.normalize_water_type(raw_text)
                        or answer_by_field.get("target_water_type", target_water_type)
                    ),
        ...
        *,
        ...
        for claim in claims:
            ...
            if (
                ...
                # 명시적 원문과 충돌하는 선택 힌트·LLM 주장은 채택하지 않는다.
                continue
            if field_name == "target_water_type":
                raw_water_type = self.normalizer.normalize_water_type(raw_text)
                if raw_water_type is not None and raw_water_type != value:
                    continue
    ...
    @staticmethod
    def _deterministic_safety_signals(raw_text, previous_answers) -> SafetySignals:
        evidence = []
        sources = [("RAW_SYMPTOM", raw_text)] + [
            ("PREVIOUS_ANSWER", str(answer.get("answer_text", "")))
            for answer in previous_answers if isinstance(answer, dict)
        ]
        for source, text in sources:
            for name, quote in detect_safety_evidence(text):
                if not any(item.signal_name == name for item in evidence):
                    evidence.append(SafetySignalEvidence(
                        signal_name=name, evidence_quote=quote, source=source,
                    ))
        return SafetySignals(**{item.signal_name: True for item in evidence}, evidence=evidence)

    @staticmethod
    def _merge_safety_signals(first: SafetySignals, second: SafetySignals) -> SafetySignals:
        evidence = list(first.evidence)
        for item in second.evidence:
            if not any(existing.signal_name == item.signal_name for existing in evidence):
        ...
        *,
        ...
        for evidence in signals.evidence:
            ...
            if not getattr(signals, signal_name):
                continue
            source_texts = [raw_text] if evidence.source == "RAW_SYMPTOM" else [
                str(answer.get("answer_text", "")) for answer in previous_answers
                if isinstance(answer, dict)
            ]
            if not any(supports_safety_quote(text, signal_name, evidence.evidence_quote)
