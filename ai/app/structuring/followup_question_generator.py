...
import re
from .followup_wording_guard import FollowUpWordingGuard
...
class FollowUpQuestionGenerator:
    ...
    def _validate_one_wording(
        ...
        *,
        ...
        try:
            FollowUpWordingGuard().validate(
                question_text, target_field=fixed.target_field, is_question=True,
            )
        except ValueError as exc:
            raise _FollowUpFieldValidationError("UNSAFE_QUESTION", str(exc)) from exc
    ...
    def _validated_options(
        *,
        ...
        try:
            for option in normalized:
                FollowUpWordingGuard().validate(option, target_field=target_field, is_question=False)
        except ValueError as exc:
            raise _FollowUpFieldValidationError("UNSAFE_OPTION", str(exc)) from exc
