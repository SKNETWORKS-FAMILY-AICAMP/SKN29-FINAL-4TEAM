...
def execute_structuring_stage(
    ...
    *,
    ...
    preflight = RiskClassifier().classify(
        ctx.raw_symptom, ctx.selected_symptoms, previous_answers=ctx.previous_answers,
    )
    if preflight.risk_level.value == "danger":
        # An external Provider must not consume the deadline before Safety.
        llm_client = None
