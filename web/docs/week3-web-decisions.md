...
## 결정 12. 위험도 최초 라우팅은 Mock 규칙과 실제 계약을 구분한다

- 화면 검증용 Mock은 `GENERAL → FIELD_TECHNICIAN`, `CAUTION·DANGER → CONSULTANT`를 사용한다.
- 문진 진행 중에는 담당 분기를 적용하지 않고 문진 완료 뒤 AI 판단 시점에 적용한다.
- 상담사 큐에는 상담사에게 현재 허용 행동이 있는 주의·긴급 문의만 표시한다.
- 실제 API 연결 뒤에는 Web이 위험도로 담당자를 다시 계산하지 않고 Backend의 상태·담당 주체·`allowed_actions`만 소비한다.

## 결정 13. API의 +09:00 wall-clock 일시는 재변환하지 않는다

- 계약 일시는 `+09:00`이 포함된 ISO 8601 값을 원천으로 사용한다.
- 화면 Formatter는 ISO 문자열의 연·월·일·시·분을 표시 형식으로만 바꾸며 브라우저 로컬 시간대나 UTC로 변환하지 않는다.
- 오전 00시대와 오후 23시대 날짜 경계 테스트를 유지한다.
